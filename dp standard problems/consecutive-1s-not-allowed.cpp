...
void solve(){
  ...
  int n;
  cin>>n;
  ll onesstarting[n+1],zeroestarting[n+1];
  onesstarting[1]=1;
  zeroestarting[1]=1;

fr(i,2,n+1){
onesstarting[i]=zeroestarting[i-1];
zeroestarting[i]=onesstarting[i-1]+zeroestarting[i-1];
}

cout<<onesstarting[n]+zeroestarting[n]<<endl;
}
