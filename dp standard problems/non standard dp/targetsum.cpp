  ...
  template<class T> ostream& operator<<(ostream& out, set<T> vec) { out<<"("; for (auto& v: vec) out<<v<<", "; return out<<")"; }
    ...
    int findTargetSumWays(vector<int>& v, int sum) {
        ...
        fr(i,0,n-1)       
        {
            ...
            if(dpp[i][j])
            {
                dpp[i+1][j+v[i+1]] += dpp[i][j];
                if(j-v[i+1]<0)
                    dpn[i+1][abs(j-v[i+1])] += dpp[i][j];
                else
                    dpp[i+1][abs(j-v[i+1])] += dpp[i][j];
            }
            if(dpn[i][j])
            {
                if(-j+v[i+1]>=0)
                    dpp[i+1][abs(-j+v[i+1])] += dpn[i][j];
                else
                   dpn[i+1][abs(-j+v[i+1])] += dpn[i][j];
        ...
        return dpn[n-1][abs(sum)];
        else
        return dpp[n-1][0] + dpn[n-1][0];
